...
//  BulletingBoard
//
//  Created by Lewis Jones on 15/03/2019.
...
class MessageSearchTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        CloudKitManager.shared.requestDiscoverabilityAuthorization { (status, error) in
            if let error = error {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(reloadViews), name: MessageController.shared.searchedUserMessagesWereUpdatedNotification, object: nil)
    }

    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        guard let email = searchBar.text else { return }
        UIApplication.shared.isNetworkActivityIndicatorVisible = true
        MessageController.shared.fetchMessagesOfUserWith(email: email)
